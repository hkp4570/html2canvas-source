...
export class StackingContext {
    ...
    negativeZIndex: StackingContext[]; // zIndex为负的元素
    zeroOrAutoZIndexOrTransformedOrOpacity: StackingContext[]; // 所有有层叠上下文的（z-index: auto|0）、透明度小于1的（opacity小于1）或变换的（transform不为none）元素
    positiveZIndex: StackingContext[]; // z-index大于等于1的元素
    nonPositionedFloats: StackingContext[]; // 未定位的浮动元素
    nonPositionedInlineLevel: StackingContext[]; // 内联的非定位元素，包含内联表和内联块
    inlineLevel: ElementPaint[]; // 内联元素
    nonInlineLevel: ElementPaint[]; // 非内联元素
...
// 树中的每一个ElementContainer节点都会产生一个ElementPaint对象，这个对象将会被直接作为第三阶段绘制该节点的依据
...
export const parseStackingContexts = (container: ElementContainer): StackingContext => {
    // container 是element 构建的ElementContainer树
    const paintContainer = new ElementPaint(container, null);
    console.log(paintContainer, 'paintContainer');
